...
// Hash: 2017a5b35c8ea5f4addb7f21ef0a5214d3eba29aa21de2414193b7b1f32dc4bb
package vm
...
type SuaveRuntime interface {
	...
	mergeBlobData(toAddresses []common.Address, blobsData [][]byte) ([][]byte, error)
...
var (
	...
	mergeBlobDataAddr            = common.HexToAddress("0x0000000000000000000000000000042042042001")
...
var addrList = []common.Address{
	buildEthBlockAddr, confidentialInputsAddr, confidentialRetrieveAddr, confidentialStoreAddr, ethcallAddr, extractHintAddr, fetchBidsAddr, fillMevShareBundleAddr, mergeBlobDataAddr, newBidAddr, signEthTransactionAddr, simulateBundleAddr, submitBundleJsonRPCAddr, submitEthBlockBidToRelayAddr,
}
...
func (b *SuaveRuntimeAdapter) run(addr common.Address, input []byte) ([]byte, error) {
	...
	case mergeBlobDataAddr:
		return b.mergeBlobData(input)
...
func (b *SuaveRuntimeAdapter) mergeBlobData(input []byte) (res []byte, err error) {
	var (
		unpacked []interface{}
		result   []byte
	)
	...
	unpacked, err = artifacts.SuaveAbi.Methods["mergeBlobData"].Inputs.Unpack(input)
	if err != nil {
		err = errFailedToUnpackInput
		return
	}
	...
	toAddresses = unpacked[0].([]common.Address)
	blobsData = unpacked[1].([][]byte)

	var (
		output1 [][]byte
	)

	if output1, err = b.impl.mergeBlobData(toAddresses, blobsData); err != nil {
		return
	}

	result, err = artifacts.SuaveAbi.Methods["mergeBlobData"].Outputs.Pack(output1)
	if err != nil {
		err = errFailedToPackOutput
		return
	}
